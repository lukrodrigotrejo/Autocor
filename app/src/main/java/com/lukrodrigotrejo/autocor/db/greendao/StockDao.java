...
public class StockDao extends AbstractDao<Stock, String> {
    ...
    */
    ...
    public List<Stock> getbyFiltros(Long rubro, String marca, Long tipo){
        List<Stock> stockList;
        QueryBuilder qb = queryBuilder();

        if(rubro != 0){
            qb.where(Properties.Rubro.eq(rubro));
        }
        if(marca != null){
            qb.where(Properties.Marca.eq(marca));
        }
        if(tipo != 0){
            qb.where(Properties.Tipo_Auto.eq(tipo));
        }
    ...
    // TODO Mostrar las marcas dependiendo el rubro seleccionado
    public List getMarcasByStock(Stock stock){
        ArrayList<String> result = new ArrayList<String>();
        Cursor c = session.getDatabase().rawQuery("SELECT DISTINTC " + Properties.Marca.columnName + " FROM " + TABLENAME, null);
        if (c.moveToFirst()) {
            do {
